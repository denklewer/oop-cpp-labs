...
void main()
{
	...
	MyString ss = s; //потому, что  вызывается конструктор копирования

	// Создайте конструктор копирования, который действительно копирует текст
	// строки, а не запоминает ее адрес. Используйте существующие методы (Copy). done.
	...
	Vector d;
	d = a + b;   // во время присвоения сбрасывается значение. ???????????????????????????????????????????????????
 ...
 stop;
	...
	w = 5 * v * atan(1.);    //результат сбрасывается при присвоении ???????????????????????????????
	w.Out();
	...
	// так как в обоих случаях левое выражение -- вектор, то можно без дружественных функций
	//=============================================
	...
	for (int i = 0; i<Shape::GetCount(); i++)    //  !! Моя программа считает количество всех объектов и связанных с ними векторов.
	{
	...
	delete pc;
	Shape::PrintCount();
	}        // у всех объектов в этом блоке кончилось время жизни, программа их удалила
	Shape::PrintCount();
	stop;
